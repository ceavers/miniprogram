// warehouse/pages/check/check.js
...
Page({
  ...
  toAddGoods() {
    wx.setStorageSync('selectCheckList', this.data.goodsList)
    wx.navigateTo({
      url: '/warehouse/pages/inStorage_add_goods/inStorage_add_goods?type=3',
    })
  },
  getNote(e) {
    this.setData({
  ...
  toHistoryList() {
    ...
    if (pages[pages.length - 2].route == 'warehouse/pages/stock_check_list/stock_check_list') {
      wx.navigateBack({
  ...
  createInOkPage() {
    ...
    const data = this.createCheckBillParams()
    api.warehouse.createCheckBill(data)
    .then(res=>{
      const billId = res.data.order_id;
      wx.navigateTo({
        url: `/warehouse/pages/create_inStorage_res/create_inStorage_res?type=3&id=${billId}`,
      })
      wx.removeStorage({
        key: 'selectCheckList',
  ...
  //盘点单参数
  createCheckBillParams(){
    let data = {
      depo_id: this.data.stockCode,
  ...
  deleteGoods(e) {
    ...
    wx.setStorageSync("selectCheckList", tempArr)
  },
  //加
  addGoodsNum(e) {
   ...
   */
  onShow: function () {
    if (wx.getStorageSync('selectCheckList')) {
      this.setData({
        goodsList: wx.getStorageSync('selectCheckList')
      })
    } else {
      this.setData({
        goodsList: [],
