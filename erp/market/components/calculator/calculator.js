...
Component({
  ...
  methods: {
    ...
    cancelSettlement(){
      ...
      this.triggerEvent('cancel', { res: 'cancel' })
    },
    comfirm(){
      if(this.properties.result){
        this.triggerEvent("setResult", { res: this.data.result })
        this.setData({
          input: '',
          result: '',
          show: false,
    ...
    handleTap(event) {
      ...
      //判断等号
        ...
        if (!isNaN(this.data.result)&&this.data.result.trim().length){
          this.setData({
            isComfirm: true
          })
        }else{
