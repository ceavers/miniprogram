...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.type) {
      this.setData({
        type: options.type,
        supplierId: JSON.parse(options.userMes).id,
        orderMoney: options.orderMoney
      })
    }
    this.getAccountList();
  ...
  createOrder(e) {
    ...
    if(this.data.type){
      api.market.addPurchaseOrder({
        type: 0,//类型 0 采购 1 退货
        user_id: 1,//本人id,
        supplier_id: this.data.supplierId,//供应商id
        date: parseInt(this.data.dateObj.timeStamp / 1000),//时间
        data: JSON.stringify(wx.getStorageSync('selectPurchaseList')),//商品
        note: e.detail.value.remark,//备注
        foreign_order_id: e.detail.value.orderNum,//外部单号
        buy_money: this.data.orderMoney,//采购金额
        already_pay_money: e.detail.value.payMoney,//实付金额
        order_image: JSON.stringify(this.data.imagesList) || '',//单据图片
        repository_type: this.data.wareHouse.depo_id,//仓库
      }).then(res => {
        if (res.code == 1) {
          wx.removeStorageSync('selectPurchaseList')
          wx.redirectTo({
            url: '/market/pages/create_new_purchase_success/create_new_purchase_success?orderId=' + res.data.order_id,
          })
        }
      })
    } else {
      api.market.postPurchaseListEntry({
        id: app.globalData.selectUser.userId,
        buy_money: e.detail.value.orderMoney,
